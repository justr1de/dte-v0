...
import { send2FACode, verify2FACode, is2FASessionValid } from '@/services/twoFactorService'
...
export default function Login() {
  ...
  const handle2FAVerified = () => {
    setShow2FA(false)
    toast.success('Verificação concluída! Bem-vindo ao sistema.')
    navigate('/dashboard')
  }

  const handle2FACancel = async () => {
    setShow2FA(false)
    // Fazer logout pois o login já foi feito mas 2FA não foi verificado
    const { signOut } = useAuth()
    // Não chamar signOut aqui pois causaria erro de hooks
    // Apenas fechar o modal e o usuário terá que fazer login novamente
    toast.info('Verificação cancelada. Faça login novamente.')
    ...
    <>
      ...
      {show2FA && (
        <TwoFactorAuth
          ...
          onSendCode={send2FACode}
          onVerifyCode={verify2FACode}
        />
      )}
          ...
          <div className="max-w-md w-full mx-auto">
            ...
            <div className="mb-6 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-emerald-500">Autenticação em Duas Etapas (2FA)</p>
                  <p className="text-xs text-[var(--text-secondary)] mt-1">
                    Após o login, um código de verificação será enviado ao seu email para garantir a segurança da sua conta.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-[var(--border-color)]">
              ...
              <p className="text-xs text-center text-[var(--text-muted)] mt-2 flex items-center justify-center gap-1">
                <span>&copy;</span> {new Date().getFullYear()} DATA-RO. Todos os direitos reservados.
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/60">Verificação em duas etapas</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
